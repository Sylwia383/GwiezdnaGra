...
class Postac : public sf::Sprite{
    ...
    float v_x_;
    float v_y_;
    int a_y_ = 0;//przyspieszenie grawitacyjne
    ...
    int zestrzelony_=-1;
    int przesuwanie_tla_tloo_=6840;
...
public:
    ...
    void start_wrogowie(const sf::Time &e, std::vector<std::unique_ptr<Wrogowie>> &wrogowie, std::vector<std::unique_ptr<sf::Sprite>> &statek,sf::Texture &stat,sf::Texture &sG1,sf::Texture &sG2,sf::Texture &sG3,sf::Texture &sG4,sf::Texture &podloga);
    ...
    void koniec_gry(const sf::Time &e,sf::RenderWindow &window,std::vector<std::unique_ptr<sf::Sprite>> &czeresnia,sf::Texture &tak,sf::Sprite &koniec,sf::Texture &wygral,sf::Texture &przegral,sf::Sound &sound);//,sf::SoundBuffer &win,sf::SoundBuffer &lose);//,sf::Sound &s_przeg);
};
